...
#define BUF_NOT_DECODED_SRC         (8) //inputdata src,只清空送进播放库但还未解码的数据，解码后的数据不清空，配合大缓存模式使用，
...
#define  PLAYM4_LOAD_QMLSR                      36  // QML渲染加载错误
#define  PLAYM4_FILEANA_NO_VIDEO                37  // 文件分析无视频信息
#define  PLAYM4_FILEANA_NO_AUDIO                38  // 文件分析无音频信息
...
typedef struct
{
    ...
    unsigned int   nDataLen;     // 数据大小
    unsigned int   nWidth;       // 画面宽
    unsigned int   nHeight;      // 画面高
    unsigned int   nType;        // 数据类型，1-T_UYVY,3-T_YV12,7-T_RGB32
    unsigned int   reserved[4];  // 保留
}QML_DISPLAY_YUV_INFO;
...
// 播放窗口类型
typedef enum tagPlayM4WindowType
{
    PLAYM4_WINDOW_DEFAULT,   ///< 默认窗口
    PLAYM4_WINDOW_QML        ///< QML窗口
}PLAYM4_WINDOW_TYPE_E;
...
//打开文件，用于PS纯音频文件场景，若视频流，nFileType返回0，若纯音频，nFileType返回1
PLAYM4_API BOOL __stdcall PlayM4_OpenFileEx(LONG nPort,LPSTR sFileName,int& nFileType);
...
PLAYM4_API LONG __stdcall PlayM4_SetStreamNoPlayEnd(LONG nPort); // 设置后不触发空包的流结束回调，再次送空包则继续触发结束回调
...
PLAYM4_API BOOL __stdcall PlayM4_SYNC_Chase(unsigned int nGroupIndex,int bChase); // 控制同步组内的包括暂停、帧进接口内是否采用追赶逻辑确保同一帧
PLAYM4_API LONG __stdcall PlayM4_SYNC_SetStreamNoPlayEnd(unsigned int nGroupIndex); // 设置后不触发空包的流结束回调，再次送空包则继续触发结束回调
...
#define    PLAYM4_AUDIO_DB     (1 << 1)     // nCBInfo第1位-音频回调DB值
...
// 注：路径传空时，默认在ShinPlayCtrl.dll同级目录生成日志文件
...
{
    char* pLogPath;          // 日志保存路径，传NULL则默认生成在ShinPlayCtrl.dll目录
    char* pStreamPath;       // 码流保存路径，传NULL则默认生成在ShinPlayCtrl.dll目录
...
#define PLAYM4_RTINFO_RENDER_FRAMERATE             0x1F000004 // 渲染帧率，用保留字节前三个保存帧率，最大值，最小值和平均值
...
PLAYM4_API BOOL __stdcall PlayM4_PlayOneFrameBySystemTime(LONG nPort, PLAYM4_SYSTEM_TIME* stSystemTime, BOOL bFlag, void* pReserved);
PLAYM4_API BOOL __stdcall PlayM4_PlayOneFrame(LONG nPort,BOOL bFlag);

//纯音频定位
PLAYM4_API BOOL __stdcall PlayM4_SetAudioPlayedTimeEx(LONG nPort,DWORD nTime);
//获取音频总时长
PLAYM4_API DWORD __stdcall PlayM4_GetAudioFileTime(LONG nPort);
